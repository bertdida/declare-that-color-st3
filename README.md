...
- There are cases that 2 or more hex codes own the same color name. For example #FFF, #FEFEFE and #FDFDFD are having the name white. If these hex codes are used within a document in order from top to bottom, the following variable names will be used during declaration respectively: `white`, `white-2` and `white-3`.
- When using the [stylus](http://stylus-lang.com/) preprocessor, the variable will be declared with dollar sign prepended on it with trailing semicolon.
